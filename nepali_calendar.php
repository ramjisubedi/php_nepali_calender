...
class Nepali_Calendar {
    ...
    public function eng_to_nep($yy, $mm, $dd) {
        ...
        if ($this->is_range_eng($yy, $mm, $dd) == false) {
            ...
            $this->nep_date["n_month_name"] = $this->get_nepali_month($m);
            $this->nep_date["num_day"] = $numDay;

            $lastTwoDigit =substr($y,2,4);
            # substr(string ,starting postion,length)
            
            $total_days = $this->bs[$lastTwoDigit][$m-1];
            $start_day = $total_days % 7;
            
            $this->nep_date["total_day"] = $this->bs[$lastTwoDigit][$m];
            $this->nep_date["start_day"] = $start_day;
    ...
    public function nep_to_eng($yy, $mm, $dd) {
        ...
        if ($this->is_range_nep($yy, $mm, $dd) === false) {
            ...
            $this->eng_date["e_month_name"] = $this->get_english_month($m);
